...
- New LXC container or use existing infrastructure
- Static IP (e.g., 192.168.1.110)
...
ExecStart=/opt/mcp-servers/.venv/bin/python -m servers.%i --transport streamable-http --host 0.0.0.0 --port %I_PORT
Restart=always
RestartSec=5
...
```bash
# On Proxmox: verify services running
systemctl status mcp-server@calculator

# From dev machine: verify connectivity
curl http://192.168.1.110:9003/mcp
