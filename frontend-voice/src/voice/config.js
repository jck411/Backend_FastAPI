...
export const VOICE_CONFIG = {
  ...
  history: {
    maxMessages: toNumber(import.meta.env.VITE_VOICE_HISTORY_LIMIT, 200),
  },
  tts: {
    defaultSampleRate: toNumber(import.meta.env.VITE_VOICE_TTS_SAMPLE_RATE, 24000),
  },
  ui: {
    timeoutCountdownSeconds: toNumber(
      import.meta.env.VITE_VOICE_TIMEOUT_COUNTDOWN_SECONDS,
      5,
    ),
  },
};
...
export const createClientId = () => {
  ...
  return `voice_${Math.random().toString(36).slice(2)}${Date.now().toString(36)}`;
};
