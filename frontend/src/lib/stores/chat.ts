    ...
    try {
      await startChatStream(payload, {
        ...
        onNotice(payload) {
          ...
          // Handle tool plan status notices
          if (type === 'tool_plan_status') {
            ...
            const fallbackReason = (payload as Record<string, unknown>).fallback_reason;
            console.log('[NOTICE] Tool plan status:', { messageText, usedFallback, fallbackReason, assistantMessageId });
            if (typeof messageText !== 'string') {
              console.warn('[NOTICE] Invalid messageText in tool_plan_status:', messageText);
              return;
            }
            store.update((value) => {
              console.log('[NOTICE] Updating store with plan notice. Current messages:', value.messages.length);
              const messages = value.messages.map((message) => {
                if (message.id !== assistantMessageId) {
                  return message;
                }
                console.log('[NOTICE] Found assistant message, updating details:', message.id);
                ...
                return {
                  ...
                  details: {
                    ...
                    planFallbackReason: typeof fallbackReason === 'string' ? fallbackReason : null,
                  },
                };
              });
              console.log('[NOTICE] Store updated');
              return { ...value, messages };
            });
            return;
          }

          // Handle tool rationale notices
          if (type === 'tool_rationale') {
            const messageText = (payload as Record<string, unknown>).message;
            const autoGenerated = (payload as Record<string, unknown>).auto_generated;
            console.log('[NOTICE] Tool rationale:', { messageText, autoGenerated });
            if (typeof messageText !== 'string') {
              console.warn('[NOTICE] Invalid messageText in tool_rationale:', messageText);
              return;
            }
            store.update((value) => {
              const messages = value.messages.map((message) => {
                ...
                const existingDetails: ConversationMessageDetails = message.details ?? {};
                const currentNotice = existingDetails.planNotice || '';
                // Append rationale to existing notice with newline separator
                const updatedNotice = currentNotice
                  ? `${currentNotice}\n${messageText}`
                  : messageText;
                console.log('[NOTICE] Appending rationale to notice:', { currentNotice, updatedNotice });
                return {
                  ...message,
                  details: {
                    ...existingDetails,
                    planNotice: updatedNotice,
                  },
                };
              });
              return { ...value, messages };
