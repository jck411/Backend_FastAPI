"""Custom MCP server for NOTES folder integration via Google Drive.

This server provides tools to interact with a NOTES folder stored in Google Drive,
...
async def delete_directory(
    ...
    # Move the folder to trash (this will also trash all contents)
    try:
        await asyncio.to_thread(
            service.files()
            .update(
...
async def move_note(
    ...
    if dest_id and not overwrite:
        return f"Destination '{destination_path}' already exists. Set overwrite=True to replace."
    ...
    # Navigate/create destination folder structure
...
async def search_vault(
    ...
    # Get all notes in the search folder
    ...
    for note in notes:
        ...
        # Content and tag search require reading the file
...
async def get_backlinks(
    ...
    # Verify target note exists and get its name
